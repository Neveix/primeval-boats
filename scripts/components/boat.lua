...
-- module system with ability of adding new boats from other packs.

local PLANK_BOAT_ID = PACK_ID .. ":plank_boat"
local LOG_RAFT_ID = PACK_ID .. ":log_raft"
local KAYAK_ID = PACK_ID .. ":kayak"

local PARAMETERS = {
	[PLANK_BOAT_ID] = {
		layout_id = PACK_ID .. ":inventory20",
		inventory_size = 20,
		gravity = 1,
		max_speed = 7,
...
local P = PARAMETERS[entity:def_name()]
local GRAVITY = P.gravity
local MAX_SPEED = P.max_speed
local ROTATION_ACCELERATION = P.rotation_acceleration
local ROTATION_DECELERATION = P.rotation_deceleration or 0.003
...
local ACCELERATION = P.acceleration or 0.04
local WATER_SPLASHES_NUMBER = P.water_splashes_number or 3
local WATER_SPLASHES_WIDTH = P.water_splashes_width or 1
local INVENTORY_SIZE = P.inventory_size
local LAYOUT_ID = P.layout_id
local PLAYER_POS_SHIFT = P.player_pos_shift or { 0, 0.8, 0 }
